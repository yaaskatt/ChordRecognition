...
def create_beat_training_data(note_name, chord_type):
    ...
    for k in range(songSet.shape[0]):
        ...
        for i in range(len(beats)):
            ...
            if (root not in note_name and root not in noteMap_dict and root != 'N') or type not in chord_type:
                print("Error, root =", root, ", type =", type)
            chord_start = float(chord_start)
            chord_end = float(chord_end)
            ...
            if k == 0 and i == 0:
                ...
                if (start_time > chord_start and end_time < chord_end) or j == len(chordSet) - 1:
